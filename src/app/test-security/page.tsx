...
export default function TestSecurityPage() {
  ...
  return (
    <SubPageLayout
      maxWidth="5xl"
      breadcrumbs={[{ label: 'Test bảo mật' }]}
      title="Test bảo mật website"
      subtitle="Kiểm tra API và SQL injection protection"
      centered={false}
    >
        <div className={`${cardShell} ${cardBody} mb-6`}>
          <div className="mb-4">
            <label className={labelModern}>Base URL:</label>
            <input
              type="text"
              value={baseUrl}
          ...
          <div className="mb-4">
            <label className={labelModern}>Account ID:</label>
            <input
              type="text"
              value={accountId}
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={testAllAPIs} className={btnPrimaryClass} style={btnPrimaryStyle} disabled={loading}>
              {loading ? 'Đang test...' : 'Test tất cả API'}
            </button>
            <button onClick={testSQLInjection} className={`${btnPrimaryClass} border border-purple-500/40 bg-transparent`} disabled={loading}>
        ...
        {results && (
          <div className={`${cardShell} ${cardBody}`}>
            <h2 className="font-display text-lg text-purple-300 mb-4">Kết quả test</h2>
